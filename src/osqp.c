...
void osqp_set_default_settings(OSQPSettings *settings) {
  ...
  settings->rho           = (float)RHO;            /* ADMM step */
  settings->sigma         = (float)SIGMA;          /* ADMM step */
  ...
  settings->adaptive_rho_tolerance = (float)ADAPTIVE_RHO_TOLERANCE;

  settings->adaptive_rho_fraction = (float)ADAPTIVE_RHO_FRACTION;

  settings->max_iter      = MAX_ITER;                /* maximum iterations to
                                                        take */
  settings->eps_abs       = (float)EPS_ABS;        /* absolute convergence
                                                        tolerance */
  settings->eps_rel       = (float)EPS_REL;        /* relative convergence
                                                        tolerance */
  settings->eps_prim_inf  = (float)EPS_PRIM_INF;   /* primal infeasibility
                                                        tolerance */
  settings->eps_dual_inf  = (float)EPS_DUAL_INF;   /* dual infeasibility
                                                        tolerance */
  settings->alpha         = (float)ALPHA;          /* relaxation parameter */
...
int osqp_setup(OSQPWorkspace** workp, const OSQPData *data, const OSQPSettings *settings) {
  int exitflag;
  ...
  *workp = work;
  ...
  work->rho_vec     = c_malloc(data->m * sizeof(float));
  work->rho_inv_vec = c_malloc(data->m * sizeof(float));
  if ( data->m && (!(work->rho_vec) || !(work->rho_inv_vec)) )
    return osqp_error(OSQP_MEM_ALLOC_ERROR);

  // Type of constraints
  work->constr_type = c_calloc(data->m, sizeof(int));
  if (data->m && !(work->constr_type)) return osqp_error(OSQP_MEM_ALLOC_ERROR);

  // Allocate internal solver variables (ADMM steps)
  work->x        = c_calloc(data->n, sizeof(float));
  work->z        = c_calloc(data->m, sizeof(float));
  work->xz_tilde = c_calloc(data->n + data->m, sizeof(float));
  work->x_prev   = c_calloc(data->n, sizeof(float));
  work->z_prev   = c_calloc(data->m, sizeof(float));
  work->y        = c_calloc(data->m, sizeof(float));
  ...
  work->Ax  = c_calloc(data->m, sizeof(float));
  work->Px  = c_calloc(data->n, sizeof(float));
  work->Aty = c_calloc(data->n, sizeof(float));

  // Primal infeasibility variables
  work->delta_y   = c_calloc(data->m, sizeof(float));
  work->Atdelta_y = c_calloc(data->n, sizeof(float));

  // Dual infeasibility variables
  work->delta_x  = c_calloc(data->n, sizeof(float));
  work->Pdelta_x = c_calloc(data->n, sizeof(float));
  work->Adelta_x = c_calloc(data->m, sizeof(float));
  ...
  if (settings->scaling) {
    ...
    work->scaling->D    = c_malloc(data->n * sizeof(float));
    work->scaling->Dinv = c_malloc(data->n * sizeof(float));
    work->scaling->E    = c_malloc(data->m * sizeof(float));
    work->scaling->Einv = c_malloc(data->m * sizeof(float));
    ...
    work->D_temp   = c_malloc(data->n * sizeof(float));
    work->D_temp_A = c_malloc(data->n * sizeof(float));
    work->E_temp   = c_malloc(data->m * sizeof(float));
  ...
  work->pol->Alow_to_A = c_malloc(data->m * sizeof(int));
  work->pol->Aupp_to_A = c_malloc(data->m * sizeof(int));
  work->pol->A_to_Alow = c_malloc(data->m * sizeof(int));
  work->pol->A_to_Aupp = c_malloc(data->m * sizeof(int));
  work->pol->x         = c_malloc(data->n * sizeof(float));
  work->pol->z         = c_malloc(data->m * sizeof(float));
  work->pol->y         = c_malloc(data->m * sizeof(float));
  ...
  work->solution->x = c_calloc(1, data->n * sizeof(float));
  work->solution->y = c_calloc(1, data->m * sizeof(float));
...
int osqp_solve(OSQPWorkspace *work) {
    ...
    if (work->settings->adaptive_rho && !work->settings->adaptive_rho_interval) {
      ...
      if (osqp_toc(work->timer) >
        ...
        if (work->settings->check_termination) {
          ...
          work->settings->adaptive_rho_interval = (int)c_roundmultiple(iter,
                                                                         work->settings->check_termination);
        } else {
...
int osqp_cleanup(OSQPWorkspace *work) {
  int exitflag = 0;
...
* Update problem data  *
************************/
int osqp_update_lin_cost(OSQPWorkspace *work, const float *q_new) {
...
int osqp_update_bounds(OSQPWorkspace *work,
                         const float *l_new,
                         const float *u_new) {
  int i, exitflag = 0;
...
int osqp_update_lower_bound(OSQPWorkspace *work, const float *l_new) {
  int i, exitflag = 0;
...
int osqp_update_upper_bound(OSQPWorkspace *work, const float *u_new) {
  int i, exitflag = 0;
...
int osqp_warm_start(OSQPWorkspace *work, const float *x, const float *y) {
...
int osqp_warm_start_x(OSQPWorkspace *work, const float *x) {
...
int osqp_warm_start_y(OSQPWorkspace *work, const float *y) {
...
int osqp_update_P(OSQPWorkspace *work,
                    const float *Px_new,
...
int osqp_update_A(OSQPWorkspace *work,
                    const float *Ax_new,
...
int osqp_update_P_A(OSQPWorkspace *work,
                      const float *Px_new,
                      const int   *Px_new_idx,
                      int          P_new_n,
                      const float *Ax_new,
...
int osqp_update_rho(OSQPWorkspace *work, float rho_new) {
  int exitflag, i;
...
****************************/
int osqp_update_max_iter(OSQPWorkspace *work, int max_iter_new) {
...
int osqp_update_eps_abs(OSQPWorkspace *work, float eps_abs_new) {
...
int osqp_update_eps_rel(OSQPWorkspace *work, float eps_rel_new) {
...
int osqp_update_eps_prim_inf(OSQPWorkspace *work, float eps_prim_inf_new) {
...
int osqp_update_eps_dual_inf(OSQPWorkspace *work, float eps_dual_inf_new) {
...
int osqp_update_alpha(OSQPWorkspace *work, float alpha_new) {
...
int osqp_update_warm_start(OSQPWorkspace *work, int warm_start_new) {
...
int osqp_update_scaled_termination(OSQPWorkspace *work, int scaled_termination_new) {
...
int osqp_update_check_termination(OSQPWorkspace *work, int check_termination_new) {
...
int osqp_update_delta(OSQPWorkspace *work, float delta_new) {
...
int osqp_update_polish(OSQPWorkspace *work, int polish_new) {
...
int osqp_update_polish_refine_iter(OSQPWorkspace *work, int polish_refine_iter_new) {
...
int osqp_update_verbose(OSQPWorkspace *work, int verbose_new) {
...
int osqp_update_time_limit(OSQPWorkspace *work, float time_limit_new) {
