...
#include ".../include/ldl_interface.h" // Include only this solver in the same directory
...
int unload_linsys_solver(enum linsys_solver_type linsys_solver) {
  ...
  case LDL_SOLVER:
    ...
    // We do not load LDL solver. We have the source.
    return 0;
...
int init_linsys_solver(LinSysSolver          **s,
  ...
  switch (linsys_solver) {
  case QDLDL_SOLVER:
    return init_linsys_solver_ldl((qdldl_solver **)s, P, A, sigma, rho_vec, polish);
