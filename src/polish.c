 ...
 */
static int form_Ared(qpWorkspace *work) {
    int j, ptr;
    int Ared_nnz = 0;
 ...
 * @return      reduced rhs
 */
static void form_rhs_red(qpWorkspace *work, float *rhs) {
    int j;
 ...
 * @return      Exitflag
 */
static int iterative_refinement(qpWorkspace *work,
                                LinSysSolver  *p,
                                float       *z,
                                float       *b) {
    int i, j, n;
    ...
    if (work->params->polish_refine_iter > 0) {
        ...
        if (!rhs) {
            //   return osqp_error(qp_MEM_ALLOC_ERROR);
            return -5; // qp mem alloc error
        } else {
            for (i = 0; i < work->params->polish_refine_iter; i++) {
 ...
 */
static void get_ypol_from_yred(qpWorkspace *work, float *yred) {
    int j;
...
int polish(qpWorkspace *work) {
    ...
    rhs_red = malloc(sizeof(float) * (work->data->n + mred));
    if (!rhs_red) {
        // Polishing failed
        work->info->status_polish = -1;
