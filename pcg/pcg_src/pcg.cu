...
__global__ void scalar_division_kernel(float       *res,
                                       const float *num,
                                       const float *den) {
...
int cuda_pcg_alg(cudapcg_solver *s,
  ...
  if (!s->warm_start) {
    /* d_x = 0 */
    checkCudaErrors(cudaMemset(s->d_x, 0, n * sizeof(float)));
  }

  /* d_p = 0 */
  checkCudaErrors(cudaMemset(s->d_p, 0, n * sizeof(float)));
  ...
  while ( *(s->h_r_norm) > eps && iter < max_iter ) {
    ...
    checkCudaErrors(cudaMemcpyAsync(s->h_r_norm, s->d_r_norm, sizeof(float), cudaMemcpyDeviceToHost));
...
void cuda_pcg_update_precond(cudapcg_solver *s,
  ...
  size_t Buffer_size_in_bytes = n * (sizeof(float) + sizeof(int));
  ...
  if (A_updated || R_updated) {
    /* Allocate memory */
    cuda_malloc((void **) &tmp, At->nnz * sizeof(float));
