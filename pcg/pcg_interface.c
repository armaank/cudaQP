...
static float compute_tolerance(cudapcg_solver *s,
                                 int           admm_iter) {
...
int init_linsys_solver_cudapcg(cudapcg_solver    **sp,
  ...
  int n, m;
  float H_MINUS_ONE = -1.0;

  /* Allocate linsys solver structure */
  cudapcg_solver *s = calloc(1, sizeof(cudapcg_solver));
  *sp = s;
  ...
  /* Set pointers to problem data and ADMM params */
  s->A            = A->S;
  s->At           = A->At;
  s->P            = P->S;
  ...
  if (!polish) {
    s->h_sigma = &params->sigma;
    s->h_rho   = &params->rho;
  }
  else {
    s->h_sigma = &params->delta;
    s->h_rho   = (float*) malloc(sizeof(float));
    *s->h_rho  = 1. / params->delta;
  }

  /* Allocate PCG iterates */
  cuda_calloc((void **) &s->d_x,   n * sizeof(float));    /* Set d_x to zero */
  cuda_malloc((void **) &s->d_p,   n * sizeof(float));
  cuda_malloc((void **) &s->d_Kp,  n * sizeof(float));
  cuda_malloc((void **) &s->d_y,   n * sizeof(float));
  cuda_malloc((void **) &s->d_r,   n * sizeof(float));
  cuda_malloc((void **) &s->d_rhs, n * sizeof(float));
  if (m != 0) cuda_malloc((void **) &s->d_z, m * sizeof(float));

  /* Allocate scalar in host memory that is page-locked and accessible to device */
  cuda_malloc_host((void **) &s->h_r_norm, sizeof(float));

  /* Allocate device-side scalar values. This way scalars are packed in device memory */
  cuda_malloc((void **) &s->d_r_norm, 8 * sizeof(float));
  s->rTy         = s->d_r_norm + 1;
  ...
  if (s->precondition) {
    cuda_malloc((void **) &s->d_P_diag_val,       n * sizeof(float));
    cuda_malloc((void **) &s->d_AtRA_diag_val,    n * sizeof(float));
    cuda_malloc((void **) &s->d_diag_precond,     n * sizeof(float));
    cuda_malloc((void **) &s->d_diag_precond_inv, n * sizeof(float));
    if (!s->d_rho_vec) cuda_malloc((void **) &s->d_AtA_diag_val, n * sizeof(float));
  }
